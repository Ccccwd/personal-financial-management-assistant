...
class WeChatBillService:
    ...
    def __init__(self):
        ...
        # 微信支出类型关键词（当"收/支"列不可用时作为 fallback）
    ...
    def parse_file(self, file_content: bytes, filename: str, preview_only: bool = False) -> ImportPreview:
        """解析微信账单文件（自动识别 CSV/XLSX 格式）"""
        ...
        return self._parse_rows(rows, filename, preview_only, "微信账单XLSX" if ext == 'xlsx' else "微信账单CSV")

    def parse_csv_content(self, csv_content: str, preview_only: bool = False) -> ImportPreview:
        """解析微信CSV账单内容（兼容旧接口）"""
        raw_bytes = csv_content.encode('latin1', errors='replace')
        rows = self._read_csv(raw_bytes)
        return self._parse_rows(rows, "wechat_bill.csv", preview_only, "微信账单CSV")

    def validate_file_format(self, file_content: bytes, filename: str) -> ValidateResponse:
        """验证文件格式"""
        ext = filename.lower().split('.')[-1] if filename else ''

        if ext not in ('csv', 'xlsx'):
            return ValidateResponse(is_valid=False, message="请上传 CSV 或 XLSX 格式文件")
        ...
        try:
            ...
            data_rows = self._get_data_rows(rows, header_row_index)
            if not data_rows:
    ...
    def validate_csv_format(self, file_content: bytes, filename: str) -> ValidateResponse:
        """验证CSV文件格式（兼容旧接口）"""
        return self.validate_file_format(file_content, filename)
    ...
    def import_transactions(self, db, user_id: int, transactions: List[dict],
        ...
        for i, txn_data in enumerate(transactions):
            try:
                ...
                failed_count += 1
                errors.append({"row": i + 1, "error": str(e), "data": txn_data})
    ...
    def _read_xlsx(self, file_content: bytes) -> List[List[str]]:
        """读取 XLSX 文件，返回字符串二维数组"""
    ...
    def _read_csv(self, file_content: bytes) -> List[List[str]]:
        """读取 CSV 文件，返回字符串二维数组"""
    ...
    def _parse_rows(self, rows: List[List[str]], filename: str,
                    preview_only: bool, format_name: str) -> ImportPreview:
        """解析行数据（CSV 和 XLSX 共用）"""
        header_row_index = self._find_header_row(rows)
        if header_row_index == -1:
        ...
        return ImportPreview(
            ...
            preview_data=parsed_transactions[:10] if preview_only else parsed_transactions,
    ...
    def _get_data_rows(self, rows: List[List[str]], header_row_index: int) -> List[List[str]]:
        """提取数据行（跳过表头前的元信息行和表尾统计行）"""
        return [
            row for row in rows[header_row_index + 1:]
            if row and len(row) > 3
            and not row[0].startswith('总')
            and not row[0].startswith('---')
            and not row[0].startswith('共')
            and row[0].strip() != ''
        ]
    ...
    def _find_header_row(self, rows: List[List[str]]) -> int:
        ...
        for i, row in enumerate(rows):
            if len(row) >= 5:
                row_text = ' '.join(str(h) for h in row)
                if "交易时间" in row_text and ("交易类型" in row_text or "金额" in row_text):
                    return i
        return -1
    ...
    def _parse_transaction_row(self, headers: List[str], row: List[str]) -> Optional[Dict[str, Any]]:
        ...
        for i, header in enumerate(headers):
            if i < len(row) and header.strip():
                ...
                transaction[key] = str(row[i]).strip()
        ...
        # 优先使用"收/支"列判断类型（xlsx 格式有此列）
        direction = transaction.get('direction', '')
        if direction in ('支出', '收入'):
            txn_type = 'expense' if direction == '支出' else 'income'
        else:
            # fallback：从交易类型推断
    ...
    def _map_type(self, wechat_type: str) -> str:
        """映射微信交易类型（当"收/支"列不可用时的 fallback）"""
