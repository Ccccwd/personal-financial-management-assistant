...
- [x] 编写 `.github/workflows/ci.yml`（创建完整工作流文件，含前后端两个并行 job）
- [x] 配置 Codecov 覆盖率上传（`frontend` flag）
- [x] 添加 README 状态徽章（CI 徽章 + 前端/后端覆盖率徽章）

### 代码适配

- [x] 配置测试命令支持 CI 模式：将 `test` 脚本更新为 `vitest run --coverage`，CI 环境直接运行即可生成 `lcov.info`
...
- [x] 处理 CI 环境差异：将测试环境从 `jsdom` 改为 `happy-dom`，解决 CI 中的 `ERR_REQUIRE_ESM` 问题
...
- **`npm ci` 而非 `npm install`**：严格按 `package-lock.json` 安装，保证 CI 与本地依赖完全一致
- **注入 `VITE_API_URL`**：测试中所有 API 调用均已 mock，该环境变量仅为占位，避免 Vite 因缺少变量而警告
...
- **Codecov flag 隔离**：通过 `flag=frontend` 和 `flag=backend` 分别上传覆盖率，可以在 Codecov 面板上单独查看前后端的覆盖趋势，避免互相影响。
